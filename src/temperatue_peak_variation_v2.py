...
if __name__ == "__main__":    
    ...
    pth = '/home/hubble/temp/slope/between_1_and_2_sigma'
    subject_results = {}
    for sub in range(0,1):#len(subjects)):
        ...
        for sson in range(len(seasons)):
            season = seasons[sson]
            peak_info = result[season]['categories']['between_1_and_2_sigma']
            temp_list = []
            for pi in range(len(peak_info)):
                ...
                if peak_duration_minutes != threshold_peak_duration:
                    ...
                    # for i, row in temperature_data_in_peak.iterrows():
                    #     ax1.annotate(f"({row['ts_datetime']},{row['sensor_status']:.2f})",  # Annotate with 'ts' and 'sensor_status'
                    #                  (row['ts_datetime'], row['sensor_status']),
                    #                  textcoords="offset points", 
                    #                  xytext=(0, 5),  # Offset to avoid overlap
                    #                  ha='center', fontsize=8, color='blue')
                    for i, row in temperature_data_in_peak.iterrows():
                        # Format the timestamp to show only hour and minute
                        time_str = row['ts_datetime'].strftime('%H:%M')
                        
                        ax1.annotate(f"({time_str},   {row['sensor_status']:.2f})",  # Annotate with formatted time and sensor_status
                                     (row['ts_datetime'], row['sensor_status']),
                                     textcoords="offset points", 
                                     xytext=(0, 5),  # Offset to avoid overlap
                                     ha='center', fontsize=8, color='blue')
