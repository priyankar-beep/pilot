...
# with open('data_september_2023_to_may_2024_with_timedelta_2_hours.pkl', 'wb') as file:
#     pickle.dump(subject_dfs, file)

# with open('data_june_2024_to_august_2024_with_timedelta_2_hours.pkl', 'wb') as file:
#     pickle.dump(subject_dfs, file)



with open('/home/hubble/work/serenade/src/data_september_2023_to_may_2024_with_timedelta_2_hours.pkl', 'rb') as file:
...
for sn in range(len(subjects)):
    ...
    # all_bathroom_temperature_peaks_count = {}
    # all_bathroom_humidity_peaks_count = {}
    temperature_data = {}
    humidity_data = {}
    ...
    for year in years:
        ...
        for month in months:
            if len(house_entrance) > 0:
                ...
                out_counts, out_timings, __ = count_virtual_out_events(house_entrance, data_subject_1, environmental_sensors, year = year, month = month, st=st, et=et)
                all_out_event_counts[year][month] = out_counts
                all_out_event_timings[year][month] = out_timings
                print(year, month, out_counts)
                # if len(out_timings) > 0:
                #     all_out_event_timings.extend(out_timings)
                
            else:
                out_counts = 0
                out_timings = pd.to_datetime('1999-01-01 00:00:00')
                
            # peak_count_temperature, _, df_with_temperature_peaks = detect_and_filter_peaks_by_time( data_subject_1, sensor_file= 'Stove_Hum_Temp_temp.csv', prominence=1.5,  year=year, month=month,  st=st,  et=et )
            # peak_count_humidity, _, df_with_humidity_peaks = detect_and_filter_peaks_by_time( data_subject_1, sensor_file= 'Stove_Hum_Temp_temp.csv', prominence=15,  year=year, month=month,  st=st,  et=et )

            peak_count_bathroom_temperature, peaks_temperature, df_bathroom_temperature_peaks = detect_and_filter_peaks_by_time( data_subject_1, sensor_file= 'Shower_Hum_Temp_temp.csv', prominence=1,  year=year, month=month,  st=st,  et=et )
            # plot_values_and_peaks(df_bathroom_temperature_peaks, peaks_temperature, value_col='sensor_status')
            peak_count_bathroom_humidity, peaks_humidity,  df_bathroom_humidity_peaks = detect_and_filter_peaks_by_time( data_subject_1, sensor_file= 'Shower_Hum_Temp_humidity.csv', prominence=3,  year=year, month=month,  st=st,  et=et )
            if len(df_bathroom_temperature_peaks) > 0 and len(df_bathroom_humidity_peaks) > 0:
                plot_combined_values_and_peaks_with_day_lines(df_bathroom_humidity_peaks, peaks_humidity, df_bathroom_temperature_peaks ,peaks_temperature,'subject-2', 'bathroom', year, month)

            # plot_values_and_peaks(df_bathroom_humidity_peaks, peaks_humidity, value_col='sensor_status')
            ...
            #     plot_signal_with_peaks(df_bathroom_humidity_peaks, peaks_humidity, df_with_temperature_peaks, peaks_temperature, 'temperature peaks', 'humidity peaks', subject_name, year, month)
    ...
    subjectwise_count[subject_name] = [all_temperature_peak_counts,all_humidity_peak_counts,all_out_event_counts,all_out_event_timings]


with open('no_tiny_out_lunch_june_2024_to_august_2024_with_timedelta_2_hours.pkl', 'wb') as pickle_file:
    pickle.dump(subjectwise_count, pickle_file)



with open('no_tiny_out_dinner_september_2023_to_may_2024_with_timedelta_2_hours.pkl', 'rb') as file:
    data1 = pickle.load(file)
    
with open('no_tiny_out_dinner_june_2024_to_august_2024_with_timedelta_2_hours.pkl', 'rb') as file:
...
merged_out_time = merge_data_dicts(data1, data2, 3)
...
for subject_name in merged_temp_peak.keys():
    ...
    plot_event_and_peak_counts(
        ...
        all_temperature_peak_counts,
        all_humidity_peak_counts,
        subject_name,  strng = 'no_tiny_dinner', location = 'kitchen'
    )
